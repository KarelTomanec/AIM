...
Image::Image(float* data, int width, int height) : width(width), height(height)
{
...
template<class T>
float Distance2D(T x, T y, T i, T j)
{
	return sqrtf(pow(x - i, 2) + pow(y - j, 2));
}


void Image::ApplyBilateralFilter(float sigmaG, float sigmaB, int iterations = 1)
{
...
#pragma omp parallel
	...
	std::cout << "Bilateral filter:             " << duration.count() << " [ms]\n";

	// Update CDF
	UpdateTransformedCDF();
}

float* Image::ReconstructImage(float* gradientX, float* gradientY)
{
	float* finalImage = new float[width * height];
	//std::fill(finalImage, finalImage + width * height, 0.5f);
	std::copy(data.get(), data.get() + width * height, finalImage);
	float* tmpImage = new float[width * height];

	for (int it = 0; it < 300; it++)
	{
		std::copy(finalImage, finalImage + width * height, tmpImage);
		for (int i = 0; i < height; i++)
		{
			//#pragma omp parallel for
			for (int j = 0; j < width; j++)
			{
				finalImage[i * width + j] = 0.0f;
				finalImage[i * width + j] += (i - 1) >= 0 ? tmpImage[(i - 1) * width + j] : tmpImage[i * width + j];
				finalImage[i * width + j] += (i + 1) < height ? tmpImage[(i + 1) * width + j] : tmpImage[i * width + j];
				finalImage[i * width + j] += (j - 1) >= 0 ? tmpImage[i * width + j - 1] : tmpImage[i * width + j];
				finalImage[i * width + j] += (j + 1) < width ? tmpImage[i * width + j + 1] : tmpImage[i * width + j];
				finalImage[i * width + j] += gradientX[i * width + j] + gradientY[i * width + j];
				//finalImage[i * width + j] -= (j + 1) < width ? gradientX[i * width + j + 1] : 0.0f;
				//finalImage[i * width + j] -= (i + 1) < height ? gradientY[(i + 1) * width + j] : 0.0f;
				finalImage[i * width + j] -= (j + 1) < width ? gradientX[i * width + j + 1] : gradientX[i * width + j];
				finalImage[i * width + j] -= (i + 1) < height ? gradientY[(i + 1) * width + j] : gradientY[i * width + j];
				finalImage[i * width + j] *= 0.25f;
			}
		}
	}
...
void Image::ImageStitching(Image& img2)
{
	...
	float* xGrad1 = new float[width * height];
	float* yGrad1 = new float[width * height];
	float* xGrad2 = new float[width * height];
	float* yGrad2 = new float[width * height];

	float* img1Data = this->DataPtr();
	float* img2Data = img2.DataPtr();
	...
	for (int i = 0; i < height; i++)
	{
		...
		xGrad1[i * width + width - 1] = 0.0f;
		xGrad2[i * width + width - 1] = 0.0f;
	...
	for (int j = 0; j < width; j++)
	{
		...
		yGrad1[(height - 1) * width + j] = 0.0f;
		yGrad2[(height - 1) * width + j] = 0.0f;
	...
	//Image imgXDerivatives(xGrad1, width, height);
	//imgXDerivatives.SavePNG("../Resources/xGradients.png");
	//Image imgYDerivatives(yGrad1, width, height);
	//imgYDerivatives.SavePNG("../Resources/yGradients.png");

	float* modifiedGradientsX = new float[width * height];
	float* modifiedGradientsY = new float[width * height];
	...
	float* finalImage = ReconstructImage(modifiedGradientsX, modifiedGradientsY);
